use dataflow_rs::{Message, ThreadedEngine, Workflow};
use serde_json::json;
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("THREADEDENGINE ASYNC BENCHMARK");
    println!("===============================");
    println!("Iterations: {}", ITERATIONS);
    println!("CPU cores: {}", num_cpus::get());
    println!();
    println!("Workers | Throughput | P50 (μs) | P90 (μs) | P95 (μs) | P99 (μs) | P99.9 (μs)");
    println!("--------|------------|----------|----------|----------|----------|------------");
    ...
    let worker_configs = vec![2, 4, 8, 10];

    for workers in worker_configs {
        let (throughput, mut latency) =
            run_async_benchmark(workflow.clone(), &sample_data, workers).await?;
...
async fn run_async_benchmark(
    ...
    // Use optimal number of async tasks (2x workers)
